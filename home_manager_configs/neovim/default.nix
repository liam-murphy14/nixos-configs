...
let
  ...
  jdtlsSetupFinal =
    builtins.replaceStrings
      [ "JDTLS_PATH_BLOCK" "LOMBOK_PATH_BLOCK" "''--ROOT_MARKERS_BLOCK" ]
      [ "${pkgs.jdt-language-server}" "${pkgs.lombok}" rootMarkersBlock ]
      jdtlsSetupRaw;

  rootMarkersBlockBemolZon = "{ '.bemol' }";
  bemolOnAttachBlockBemolZon = "on_attach = bemol,";
  jdtlsSetupFinalBemolZon =
    builtins.replaceStrings
      [ "JDTLS_PATH_BLOCK" "LOMBOK_PATH_BLOCK" "''--ROOT_MARKERS_BLOCK" "--BEMOL_ON_ATTACH_BLOCK" ]
      [
        "${pkgs.jdt-language-server}"
        "${pkgs.lombok}"
        rootMarkersBlockBemolZon
        bemolOnAttachBlockBemolZon
      ]
      jdtlsSetupRaw;

  pathToJdtlsConfig =
    if pkgs.stdenv.isDarwin then
      pkgs.jdt-language-server + /share/java/jdtls/config_mac/config.ini
    else
...

{
  ...
  config = {
    ...
    home.file.".config/nvim/ftplugin/java.lua".text =
      if config.nix_neovim.enableZonBemol then jdtlsSetupFinalBemolZon else jdtlsSetupFinal;
